...
from app.database import init_db, save_analysis, list_analyses, get_analysis, delete_analysis, delete_all_analyses, update_analysis
...
async def api_analyze(
    ...
    try:
        ...
        b_show_boxes = (show_boxes in ["on", "true", True])
        ...
        if selected_classes:
            try:
                ...
                parsed_classes = [c.strip().lower() for c in selected_classes.split(',') if c.strip()]

        result = process_image(image_bytes, show_boxes=b_show_boxes, selected_classes=parsed_classes, seg_model=seg_model)
...
@app.post("/api/recalculate", response_class=JSONResponse)
async def api_recalculate(request: Request):
    try:
        data = await request.json()
        analysis_id = data.get('analysis_id')
        image_b64 = data.get('image_b64')
        seg_model = data.get('seg_model', 'unet')
        show_boxes = data.get('show_boxes', True)

        if not image_b64:
            return JSONResponse({'error': 'Missing image_b64'}, status_code=400)

        image_bytes = base64.b64decode(image_b64)
        result = process_image(image_bytes=image_bytes, seg_model=seg_model, show_boxes=show_boxes)

        if result.get('error'):
            return JSONResponse({'error': result['error']}, status_code=500)

        if analysis_id:
            update_analysis(analysis_id=int(analysis_id), total_length=result['total_length'],
                          total_area=result['total_area'], class_metrics=result['class_metrics'],
                          segmented_bytes=result['annotated_image_bytes'])
        ...
        return JSONResponse({
            ...
            'class_metrics': result['class_metrics'],
            'annotated_b64': base64.b64encode(result['annotated_image_bytes']).decode(),
            'original_b64': base64.b64encode(result['original_image_bytes']).decode(),
            'class_name': result.get('class_name', 'Unknown')
        })
    except Exception as e:
        logging.error(f"Recalculate error: {e}", exc_info=True)
        return JSONResponse({'error': str(e)}, status_code=500)
...
async def api_export(request: Request):
    try:
        ...
        rows = [get_analysis(i) for i in analysis_ids]
        rows = [r for r in rows if r is not None]
        
        if not rows:
            return JSONResponse({'error': 'No analyses found'}, status_code=404)
        ...
        with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zip_file:
            ...
            writer.writerow(['ID', 'Date', 'Filename', 'Plant', 'Model', 'Total Length (mm)', 'Total Area (mm2)', 'Class Metrics'])
            for row in rows:
                writer.writerow([row['id'], row['timestamp'], row['filename'], row['plant_class'], row['seg_model'],
                               f"{row['total_length']:.2f}", f"{row['total_area']:.2f}", json.dumps(row.get('class_metrics', {}), ensure_ascii=False)])
...
async def db_gallery(request: Request, plant: str = ""):
    ...
    return templates.TemplateResponse("db.html", {"request": request, "rows": rows, "plant_types": plant_types, "active_plant": plant})
